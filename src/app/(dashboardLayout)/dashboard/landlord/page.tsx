...
const handleSubmit = async () => {
  ...
  const uploadImages = async (): Promise<string[]> => {
    ...
    for (const file of selectedFiles) {
      ...
      const res = await fetch("https://api.cloudinary.com/v1_1/dal1rjdwl/image/upload", {
        method: "POST",
        body: formData,
      });
...
const [editData, setEditData] = useState<Listing>({
  ...
  const handleEditSubmit = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/landlords/listings/${editData._id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`http://localhost:5000/api/landlords/listings/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="flex min-h-screen text-white">
      ...
      <div className="w-64 bg-gray-900 p-6 space-y-4">
        ...
        <button className="block w-full text-left px-2 py-1 rounded">⚙️ Settings</button>
      </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {listings.map((listing) => (
                <div key={listing._id} className="bg-gray-800 p-4 rounded shadow">
                  ...
                  <div className="mt-2 flex gap-2">
                    <button onClick={() => handleUpdate(listing._id)} className="bg-blue-600 px-3 py-1 rounded">Edit</button>
                    <button
            ...
            <div className="space-y-4">
              {requests.map((req) => (
                <div key={req._id} className="bg-gray-800 p-4 rounded shadow">
                  ...
                  <p><strong>Message:</strong> {req.additionalMessage}</p>
                  {req.status === "pending" && (
                    <div className="mt-2 space-x-2">
                      <button  onClick={() => openModal(req._id)} className="bg-green-600 px-3 py-1 rounded">Approve</button>
...
{showModal2 && (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
    <div className="bg-white text-black p-6 rounded w-96">
      ...
      <div className="flex justify-end gap-2">
        <button onClick={closeModal} className="px-3 py-1 bg-gray-400 rounded">
          Cancel
        </button>
        <button
